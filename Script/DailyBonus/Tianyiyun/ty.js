// 引用地址: https://github.com/MCdasheng/QuantumultX/blob/main/Scripts/myScripts/ty.js
// 提交于 2023年8月22日
...
var session = $.getdata("ty_session");
if (!session) {
  $.log("⚠️信息不全!");
  $.log("请通过重写获取信息");
...
async function signIn() {
  ...
  return $.http.get(options).then((resp) => {
    var reg = /\<resultTip\>(.*)\<\/resultTip\>/;
    if (resp.body.search(reg) != -1) {
      var prize = reg.exec(resp.body)[1];
      $.log("🎉签到成功!");
      $.log(prize);
      $.msg($.name, "🎉签到成功!", prize);
    } else {
